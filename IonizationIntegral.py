...
Sweeps a voltage range and plots, for every (config, p, d) combination:
  ...
  • ∫ max(Re(λ_max(RV⁻¹)), 0) dx — integral of the leading eigenvalue of the
                                   transport matrix (shown only when the mechanism
                                   exposes get_R and get_V)
...
def aed_integral(EN_ref, p_val, d_val, mod, T, field_dist: FieldDistribution, N: int):
    ...
    try:
        ...
        ds     = xi_cross * d_val / N
        diff   = np.array([mod.alpha(en, p_val, T) - mod.eta(en, p_val, T)
                            for en in EN_arr])
...
def _print_data_file_table(records, has_matrix):
    ...
    for lbl, rows in groupby(records, key=lambda r: r[0]):
        ...
        hdr = (f"  {'p (bar)':>10}    {'V (kV)':>12}    {'E/N (Td)':>12}"
               f"    {'∫max(α−η,0)dx (m⁻¹)':>22}")
        if has_matrix:
        ...
        for _, p_bar, d_mm, V_kV, EN_ref, aed_val, eig_val in rows:
            ...
            if has_matrix:
                eig_str = f"{eig_val:>22.6e}" if np.isfinite(eig_val) else f"{'NaN':>22}"
                row += f"    {eig_str}"
            print(row)
    print()
...
def main():
    ...
    if _data_file_mode:
        if args.pressure_column is None or args.voltage_column is None:
            parser.error("--data-file requires both --pressure-column and --voltage-column.")
    else:
        if not args.p:
    ...
    has_matrix = all(
        hasattr(m, "get_R") and hasattr(m, "get_V")
        for _, m in curve_specs
    )
    ...
    if args.single_voltage is not None:
        ...
        for cfg_label, mod in curve_specs:
            for p_bar in args.p:
                for d_mm in args.d:
                    ...
                    aed_val = aed_integral(EN_ref, p_bar, d_m, mod, args.T, _field_dist, _N)
                    eig_val = (eig_integral(EN_ref, p_bar, d_m, mod, args.T, _field_dist, _N)
                               if has_matrix else float('nan'))
        ...
        for lbl, rows in groupby(records, key=lambda r: r[0]):
            ...
            print(hdr)
            for _, p_bar, d_mm, V_kV, EN_ref, aed_val, eig_val in rows:
                aed_str = f"{aed_val:>22.6e}" if np.isfinite(aed_val) else f"{'NaN (overflow)':>22}"
                row = f"  {V_kV:>12.4f}    {EN_ref:>12.2f}    {aed_str}"
                if has_matrix:
                    eig_str = f"{eig_val:>22.6e}" if np.isfinite(eig_val) else f"{'NaN (overflow)':>22}"
                    row += f"    {eig_str}"
                print(row)
        print()
        ...
        if args.write_to_file:
            ...
            _write_results(args, sv_curve_data, raw_dicts, mech_name, _field_dist, _N, has_matrix)
        return
    ...
    if _data_file_mode:
            ...
            for d_mm in args.d:
                ...
                aed_arr = np.array([
                    aed_integral(
                        ...
                        p_bar, d_m, mod, args.T, _field_dist, _N,
                    )
                ...
                for p_bar, V_kV, aed_val, eig_val in zip(
                    p_arr, V_arr_file, aed_arr,
                    eig_arr if eig_arr is not None else np.full_like(aed_arr, float('nan'))
                ):
                    EN_ref = V_kV * 1e3 * _kB * args.T / (p_bar * d_m * 1e-16)
                    all_records.append((label, p_bar, d_mm, V_kV, EN_ref, aed_val, eig_val))

        _print_data_file_table(all_records, has_matrix)

        if args.write_to_file:
            _write_results(args, file_curve_data, raw_dicts, mech_name,
                           _field_dist, _N, has_matrix,
        ...
        if not args.no_plot:
            ...
            for idx, (lbl, V_file, aed_arr, eig_arr) in enumerate(curve_data):
                ...
                ax.plot(V_file[mask], aed_arr[mask], color=color, ls='-',
                        marker=_markers[idx % len(_markers)], markersize=5,
                        label=rf"{lbl}  ($\alpha-\eta$)")
                if eig_arr is not None:
                    mask2 = np.isfinite(eig_arr)
                    ax.plot(V_file[mask2], eig_arr[mask2], color=color, ls='--',
                            marker=_markers[idx % len(_markers)], markersize=5,
    ...
    # ---- Write to file --------------------------------------------------
    if args.write_to_file:
        _write_results(args, file_curve_data, raw_dicts, mech_name, _field_dist, _N, has_matrix)
    ...
    if not args.no_plot:
        ...
        ax.set_ylabel(r"Ionisation integral  (m$^{-1}$)")
        if single_pd:
            title = (f"{mech_name},  p = {args.p[0]} bar,  d = {args.d[0]} mm,"
        ...
        for idx, (label, V_kV_arr, aed_arr, eig_arr) in enumerate(curve_data):
            ...
            line, = ax.plot(
                V_kV_arr[mask_aed], aed_arr[mask_aed],
                color=color, ls='-',
            ...
            if np.any(~mask_aed):
                ax.axvline(V_kV_arr[~mask_aed][0], color=color, ls=':', lw=1.0, alpha=0.7)

            # λ_max curve (dashed), same color
            if eig_arr is not None:
                mask_eig = np.isfinite(eig_arr)
                ax.plot(
                    V_kV_arr[mask_eig], eig_arr[mask_eig],
                    color=color, ls='--',
                    marker=_markers[idx % len(_markers)],
                    markevery=markevery, markersize=5,
                ...
                if np.any(~mask_eig):
                    ax.axvline(V_kV_arr[~mask_eig][0], color=color, ls=':', lw=1.0, alpha=0.4)
...
def _write_results(args, curve_data, raw_dicts, mech_name, field_dist: FieldDistribution,
                   N: int, has_matrix: bool = False, data_file=None):
